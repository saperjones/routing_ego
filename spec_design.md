...
- **Compare-all toggle (`#compare-toggle`, in the playback bar):** when ticked, the single driver view is replaced by a horizontal row of **five panels** (`#compare-fig`), one per strategy — `Raw` / `Centered` / `Smoothed` / `Human` / `Human centered` — each rendering the **same frame and the same live corner/window sliders** through its own `ProjectConfig.strategy`. Every panel reuses the identical renderer (green generated path + orange dashed real-trajectory overlay + car), so the strategies can be compared side-by-side while scrubbing or playing. The **perspective 3D** toggle applies to the compare panels too: ticking it repaints all five as windshield views (both toggles live together in the playback bar, always reachable). Unticking compare restores the single driver view.
- **Test-offline button (`#btn-offline`, Real-data tab only):** runs the Python `project_route` offline for the selected dataset + algorithm via `POST /api/offline` (served by `parking_proj.viewer_server`), then overlays the returned per-frame **Python offline path (solid green)** against the **live JS-twin path (dashed blue)** in the driver view — a visual parity check. Shows "Start processing…" then "Done — you can check the results". Disabled while compare-all is on (offline shows one algorithm) and on the Simulation tab; changing the algorithm/sliders/case clears the loaded offline result. The offline CLI is `offline_processing_routing_projection.py` (see below).
- **Controls:** play/pause, step ±1, scrubber, speed ×1/×2/×5/×10, compare-all toggle, test-offline button. Selecting a case loads its JSON and resets to frame 0.
